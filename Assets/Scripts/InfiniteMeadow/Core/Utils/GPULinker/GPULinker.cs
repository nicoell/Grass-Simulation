...
namespace InfiniteMeadow.Core.Utils.GPULinker
{
	...
	public abstract class GPULinker
	{
		protected readonly Dictionary<UpdateRate, List<IGPULink>> LinkedDictionary;

		protected GPULinker()
		{
			LinkedDictionary = new Dictionary<UpdateRate, List<IGPULink>>();

			foreach (UpdateRate rate in Enum.GetValues(typeof(UpdateRate))) LinkedDictionary.Add(rate, new List<IGPULink>());
		}
